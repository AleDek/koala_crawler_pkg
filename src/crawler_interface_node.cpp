...
{   
    ...
    ros::Publisher force_center_pub = nh.advertise<std_msgs::Float32>("/crawler/force_center", 10); 
    ros::Publisher force_left_pub = nh.advertise<std_msgs::Float32>("/crawler/force_left", 10); 
    ros::Publisher force_right_pub = nh.advertise<std_msgs::Float32>("/crawler/force_right", 10); 
    std_msgs::Float32 msg_l, msg_r, msg_c; 
   ...
   boost::thread consoleInput_th(inputTask);
    ...
    while(running){ 
        ...
        msg_c.data = fb.center.force;
        msg_l.data = fb.left.force;
        msg_r.data = fb.right.force;
        force_center_pub.publish(msg_c);
        force_left_pub.publish(msg_l);
        force_right_pub.publish(msg_r);
